...
// function return the ratio between base rate and destination rate
...
func (e *ExchangeRates) getRates() error {
	//requesting to the bank api to get rates
	...
	// decode the respone which is in XML format as refrence to our cubes struct
	xml.NewDecoder(resp.Body).Decode(&md)

	for _, c := range md.CubeData {
		r, err := strconv.ParseFloat(c.Rate, 64) // 64 is bitzise , converting string to 64 bit float
		if err != nil {
			return err
		}
...
// defining structures similer to data we get as response form bank api  = XML file
type Cubes struct {
